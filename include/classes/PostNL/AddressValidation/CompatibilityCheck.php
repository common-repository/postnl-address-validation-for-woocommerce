...
class CompatibilityCheck {
	...
	public function is_plugin_compatible(): bool {
		...
		if ( $this->is_plugin_woo_postnl_compatible() === false ) {
			//Config not compatible and display notification
		}
	 ...
	 * @return bool
	 */
	public function is_plugin_woo_postnl_compatible(): bool {

		if ( class_exists( '\PostNLWooCommerce\Shipping_Method\Settings' ) ) {
			//Load compatibility settings
			( new Compatibility\CompatibilityPostNLWooCommerce() )->init();

			//Display notification
			$woo_postnl_settings = \PostNLWooCommerce\Shipping_Method\Settings::get_instance();
			if ( $woo_postnl_settings->is_reorder_nl_address_enabled() ) {
				add_action( 'admin_notices', array( $this, 'plugin_woo_postnl_compatible' ) );
	...
	/**
	 * Plugin setting compatibility settings
	 * @return void
	 */
	public function plugin_woo_postnl_compatible() {
		$error = __('You are using the "PostNL Address Validation for WooCommerce" plugin in combination with the "PostNL for WooCommerce" plugin. You need to disable the option "Use PostNL address fields" in the "PostNL for WooCommerce" plugin.','postnl-wc-address-validation');
		$this->display_notification( $error );
	}
